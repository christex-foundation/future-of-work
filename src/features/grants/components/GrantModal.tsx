  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent
        ...
        className="inset-x-0 bottom-0 !max-h-none max-w-xl overflow-hidden !overflow-y-visible  border border-[#E6DCC9] bg-[#FBF7EF] p-0  sm:top-[50%] sm:right-auto sm:bottom-auto sm:left-[50%] sm:translate-x-[-50%] sm:translate-y-[-50%] sm:"
        ref={modalRef}
      ...
      >
        <ScrollArea className="h-full max-h-[100svh] md:h-auto md:max-h-[90svh]">
          <X
            className="absolute top-7 right-4 z-10 h-4 w-4 cursor-pointer text-[#5C5147] sm:top-6"
            onClick={onClose}
          />
