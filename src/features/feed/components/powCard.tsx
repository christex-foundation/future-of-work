...
export function PowCard({ pow, type, commentCount }: PowCardProps) {
  ...
  const actionLinks = (
    <>
      <div className="flex">
        <p
          className={`text-xs font-semibold break-all text-[#2C3A2E] sm:text-sm md:text-base ${isViewProjectHovered ? '' : 'group-hover:underline'} line-clamp-1 group-hover:decoration-current`}
        >
          {pow?.title}
        </p>
      </div>
